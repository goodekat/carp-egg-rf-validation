...
eggdata16 <- read.csv("../data/eggdata16.csv")
eggdata1415 <- read.csv("../data/eggdata1415.csv")
eggdata141516 <- read.csv("../data/eggdata141516.csv")
...
eggdata16 <- eggdata16 %>%
  ...
  mutate_at(.vars = c("Year", "Month", "Julian_Day"),
            .funs = as.integer) %>%
  mutate_at(
    .vars = c(
      "Temperature",
...
vi_plot_data <-
  ...
  mutate(
    variable = fct_recode(
      ...
      "Embryo average (mm)" = "Embryo Ave",
      "Embryo standard deviation (mm)" = "Embryo SD",
      "Embryo coefficient of variation" = "Embryo CV",
      "Membrane average (mm)" = "Membrane Ave",
      "Membrane standard deviation (mm)" = "Membrane SD",
      "Membrane coefficient of variation" = "Membrane CV",
      "Embryo to Membrane Ratio" = "Embryo to Membrane Ratio",
...
vi_plot <-
  ...
  labs(x = "Variable importance rank",
...
The following code computes the change in average probability of classification of an invasive carp based on a specified change in average membrane diameter and average embryo diameter. 
...
# Change in embryo average
pd141516 %>% 
  filter(variable == "Embryo_Ave") %>%
  mutate(value = as.numeric(as.character(value))) %>%
  ...
  mutate_at(
    ...
    .funs = function(.) {
      fct_recode(
        ...
        "Embryo average (mm)" = "Embryo_Ave",
        "Membrane standard deviation (mm)" = "Membrane_SD",
        "Water temperature (C)" = "Temperature",
        "Julian day" = "Julian_Day",
        "Embryo_to_Membrane_Ratio" = "Embryo_to_Membrane_Ratio",
        "Membrane coefficient of variation" = "Membrane_CV",
        "Conductivity (\U00B5S/cm)" = "Conductivity",
        "Embryo standard deviation (mm)" = "Embryo_SD"
      )
    }
  ) %>%
